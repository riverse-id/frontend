...
const RiverCardItem = React.memo(function RiverCardItem({
  ...
  return (
    <motion.div
      ...
      className="absolute w-48 sm:w-56 md:w-64 rounded-2xl overflow-hidden bg-white/90 p-2 shadow-2xl border border-sky-200/80 backdrop-blur-xl pointer-events-auto hover:scale-105 hover:z-40 transition-all duration-300 will-change-transform"
    >
      <div className="relative h-28 sm:h-36 md:h-40 w-full rounded-xl overflow-hidden group">
        <Image
          src={card.src}
          alt={card.title}
          fill
        ...
        <div className="absolute top-2 left-2">
          <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold shadow-lg backdrop-blur-md border border-white/20 ${card.tagBg}`}>
            {card.tag}
          </span>
        </div>
        ...
        <div className="absolute bottom-2 left-2.5 right-2.5 flex items-end justify-between text-white">
          <div className="flex flex-col max-w-[80%]">
            <span className="text-[11px] font-bold leading-tight drop-shadow truncate">{card.title}</span>
            <span className="flex items-center text-[9px] text-sky-200 gap-1 mt-0.5">
...
export default function ScrollIntroHero() {
  ...
  const scrollToBeranda = () => {
    ...
    if (berandaEl) {
      berandaEl.scrollIntoView({ behavior: "smooth" });
    }
  };

  // Automatically scroll fast to /beranda when intro scroll finishes (progress > 0.86)
  useMotionValueEvent(smoothProgress, "change", (latest) => {
    if (latest > 0.86 && !autoScrolledRef.current) {
  ...
  const darkVignetteOpacity = useTransform(smoothProgress, [0.52, 0.68, 0.88, 0.96], [0, 1, 1, 0]);
  const ambientBackgroundOpacity = useTransform(smoothProgress, [0.72, 0.90], [1, 0]);

  // White Light Aperture Circle Scaling
  const apertureScale = useTransform(smoothProgress, [0.58, 0.82, 0.92], [0.1, 4.5, 60.0]);
  const apertureOpacity = useTransform(smoothProgress, [0.55, 0.68, 0.90, 0.96], [0, 1, 1, 0]);

  // Portal message in the light
  const portalTextOpacity = useTransform(smoothProgress, [0.64, 0.74, 0.84], [0, 1, 0]);
    ...
    <div ref={containerRef} className="relative h-[250vh] w-full bg-navy selection:bg-[#0284C7] selection:text-white">
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center bg-navy">
        ...
        <motion.div
          ...
          className="absolute z-35 flex flex-col items-center text-center px-6 pointer-events-auto cursor-pointer will-change-transform"
        >
        ...
        >
          <span className="text-[11px] font-bold text-slate-700 tracking-widest uppercase bg-white/80 backdrop-blur-md px-3.5 py-1 rounded-full border border-sky-200 shadow-sm">
            Scroll Ke Bawah
          </span>
          <motion.div
            animate={{ y: [0, 8, 0] }}
